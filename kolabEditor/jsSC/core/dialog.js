...
var spellWin = {dialog_win: window, resumeAfterEditing: function() {
		this.pickup();
		spellWin.regroup(false);
	}, isStillOpen: function() {
		return window.document != null;
	}, providerID: null, provider: function() {
		if (!livespell) {
			this.handshake();
		}
		return livespell.spellingProviders[this.providerID];
	}, lang: {localize_ui: function() {
		...
		}}, allDone: false, docs: [], wordstocheck: [], currentDoc: 0, currentToken: 0, currentReason: "", editcount: 0, suggestionsInMotion: true, undo: {bookmarks: [], set: function(action) {
			var mem = {};
			...
			return aout;
		}}, tokens: {value: [], countWords: function() {
			var num = 0;
			...
			$$appeture.max = $$max;
			return $$appeture;
		}}, actions: {registerclose: function() {
			...
			if (!spellWin.provider().ShowSummaryScreen && spellWin.provider().isUniPacked && spellWin.undo.bookmarks.length == 0) {
				...
				try {
					window.parent.Modalbox.show(srthtml, {title: spellWin.lang.fetch("WIN_TITLE"), slideDownDuration: 0.2, slideUpDuration: 0.2});
					doret = true;
				} catch (e) {
				}
				try {
					window.opener.Modalbox.show(srthtml, {title: spellWin.lang.fetch("WIN_TITLE"), slideDownDuration: 0.2, slideUpDuration: 0.2});
					doret = true;
				} catch (e) {
				}
				try {
			...
			spellWin.moveNext();
		}}, optionsMenu: {showMultiLang: function(flag) {
	...
	}, querystring: {get: function(request) {
		...
		}, editingNow: false, showEdit: function(flag) {
...
window.iclose = function() {
	...
	if (spellWin.provider().WindowMode.toLowerCase() == "jquery.ui") {
