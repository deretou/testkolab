...
class UserController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('create','usernameChecker','commonPassword','createAfterInvitation','timezoneoffsettest'),
				'users'=>array('*'),
			),
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('squareView','getInviteInSquareInfos','notifBackClosingManager','setchrono',
                                                 'inviteInSquare','searchGeneral','setUserSession','chatmsgmanager','deleteSquare',
                                                 'getMyIdentity','getNotifications','contactFriends','getSquareInfo','getBinderList','addsquaretobinder',
                                                 'updateSquare','updateBinder','inviteFriends','deleteBinder','getChatConversation','userbsInformation',
                                                 'getSquareToken','update','getBinderInfo','main','newMessage','help','feedback','lockerupdate',
                                                 'locker','binderListChange','createSquare','social','connectGmail','kolabInvitationNotifDecision',
                                                 'contactsGmail','connectYahoo','getbasicinfos','createBinder','createContact'),
	 ...
	 * 
	 */
	public function actionAddsquaretobinder()
	{
           if(Yii::app()->user->isGuest || !isset(Yii::app()->session['userid'])){
                  echo(json_encode(array("status"=>"Failed","action"=>"refresh")));      
           }else{
                  if(isset($_POST['binderID'])){                  
                      $notif= Notifications::model()->findByPk($_POST['notifID']);
                      if($notif){
                       $binderid=substr($_POST['binderID'],3);    
                       $targetedSquare=  Squares::model()->findByPk($notif->squareID);
                       $criteria = new EMongoCriteria;  
                       $criteria->binderID =$binderid; 
                       $criteria->userID =Yii::app()->session[Yii::app()->session['userid'].'_user']; 
                       $targetedBinder = Binders::model()->find($criteria);
                       $date= new DateTime(gmdate('Y-m-d H:i:s'));                                                          
                       $dateCre=$date->format('Y-m-d H:i:s');
                       if($targetedSquare && $targetedBinder){
                       //-------------------------------------------------------------
                       $bindersSquareList= new SquaresList(); 
                       $bindersSquareList->squareID=$targetedSquare->squareID;
                       $bindersSquareList->squareName=$targetedSquare->name;
                       $bindersSquareList->creationDate=$targetedSquare->creationDate;
                       $bindersSquareList->lastUpdate=$dateCre;
                       $bindersSquareList->status='active';
                       if(isset($targetedSquare->deadlineTimestamp) && !is_null($targetedSquare->deadlineTimestamp)){
                       $bindersSquareList->deadlineTimestamp= $targetedSquare->deadlineTimestamp;
                       }             
                       $targetedBinder->squaresList[]=$bindersSquareList;
                       //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                       $members= new SquareMembers();
                       $members->binderID=$binderid;  
                       $availabledcolorsTemp=explode(',',$targetedSquare->availabledcolors);                       
                       $members->userColor=$availabledcolorsTemp[0];  
                       unset($availabledcolorsTemp[0]);
                       $targetedSquare->availabledcolors=  join(',', $availabledcolorsTemp);
                       $members->userRole="member";
                       $members->userID=Yii::app()->session[Yii::app()->session['userid'].'_user'];
                       $fromPicModel=User::model()->findByPk(Yii::app()->session[Yii::app()->session['userid'].'_user']); 
                       $members->firstname= $fromPicModel->firstname;
                       $members->lastname= $fromPicModel->lastname;
                       $members->dateAdhesion=$dateCre;
                       $members->photoProfile= $fromPicModel->photoProfile;
                       $targetedSquare->lastChangeDate=$dateCre;
                       $targetedSquare->squareMembers[]=$members;
                       //-----------------------------------------------------------------------------------------
                       
                       if($targetedSquare->update() && $targetedBinder->update()){
                           //---------------------------------------------------------------------
                            $helper= new UserManager();     
                            $helper->newMemberInSquare($notif->squareID);
                           //-----------------------------------------------------------------------
                           $notif->status='read';
                           $notif->decisionDate=$dateCre;
                          
                           if($notif->update()){
                             $bindersSquareList2=array();
                             $bindersSquareList2['id']=$targetedSquare->squareID;
                             $bindersSquareList2['name']=$targetedSquare->name;
                             $bindersSquareList2['datecreation']=gmmktime($targetedSquare->creationDate);
                             $binder=$_POST['binderID'];
                             if($targetedBinder->name=='Personal'){
                              $binder='klb';   
                             }                               
                             echo(json_encode(array("status"=>"Success","square"=>$bindersSquareList2,"binderid"=>$binder)));         
                           }else{
	 ...
	 * 
	 */
	public function actionSetchrono()
	{
		
              if(isset($_POST['tz'])){                  
                  $summer = new DateTime('now', new DateTimeZone($_POST['tz'])); 
                  Yii::app()->session['usertimezoneoffset']=$summer->getOffset()/3600;
                  Yii::app()->session['usertimezone']=$_POST['tz']; 
                  
                  echo(json_encode(array("status"=>"Success","timestamp"=>$summer->getTimestamp()))); 
                }else{
	...
	public function actionNewMessage()
	{
                ...
                if(isset($_POST['userinvitecounter'])){                  
                 $total=$_POST['userinvitecounter'];
                 $index=0;
                 $currentUserModel=User::model()->findByPk(Yii::app()->session[Yii::app()->session['userid'].'_user']); 
                 if($currentUserModel){
                   while ($total>0) {
                    $index++; 
                    $temp="email".$index;
                    $criteria = new EMongoCriteria;  
                    $criteria->photoProfile =$_POST[$temp];  
                    ...
                    if($userModel){
                      ...
                      //------------------------------------------------
                       $messageID=(String)($id + hexdec($crc64) + hexdec($crc));
                      $messageModel->messageID=$messageID;
                      $messageModel->createDate=$dateCre;
                      $messageModel->body=$_POST['message'];
                      $messageModel->subject=$_POST['subject'];
                      $messageModel->fromID=Yii::app()->session[Yii::app()->session['userid'].'_user'];
                      $messageModel->fromPic=$currentUserModel->photoProfile;
                      $messageModel->fromFullName=$currentUserModel->firstname.' '.$currentUserModel->lastname;
                      $messageModel->toID=$userModel->userID;
                      $messageModel->toPic=$userModel->photoProfile;
                      $messageModel->toFullName=$userModel->firstname.' '.$userModel->lastname;
                      $messageModel->timestamp=$date->getTimestamp();
                      ...
                      if($messageModel->save()){
                       //---------------------------------------------------------
                        ...
                        $crc6no =hash('crc32', $_POST['message']); 
                        $safeTokenno= $safaty->generate_token(25);
                        $crcno =hash('crc32', $safeTokenno);
                        $notification->notifID= (String)($idnotif + hexdec($crc64no) + hexdec($crc6no) + hexdec($crcno) + $messageID);
                        $notification->creationDate=$dateCre;
                        $notification->notifColor='nada';
                        $notification->status='unread';
                        $notification->notifType='newmessage';                                 
                        $notification->userID= $userModel->userID; 
                        $notification->squareID=$messageID;
                        $notification->notifText=$_POST['subject'];                  
                        $notification->from=$currentUserModel->firstname.' '.$currentUserModel->lastname;
                   ...
                   echo(json_encode(array("status"=>"Success","loopnumber"=> $index)));
                 }else{
                  echo(json_encode(array("status"=>"Failed1")));   
                 }
	 ...
	 * 
         * 
	 */
	public function actionGetChatConversation()
	{
			     if(Yii::app()->user->isGuest || !isset(Yii::app()->session['userid'])){
                                 echo(json_encode(array("status"=>"Failed","action"=>"refresh")));      
                             }else{
                             if(isset($_POST['interlocID'])){
                                $c1 = new EMongoCriteria;
                                $c1->fromID=Yii::app()->session[Yii::app()->session['userid'].'_user'];
                                $c1->toID=$_POST['interlocID'];
                                $chatMsg1 = ChatConversation::model()->findAll($c1);
                                //-------------------------
                                $c2 = new EMongoCriteria;
                                $c2->fromID=$_POST['interlocID'];
                                $c2->toID=Yii::app()->session[Yii::app()->session['userid'].'_user'];
                                $chatMsg2 = ChatConversation::model()->findAll($c2);
                                //---------------------------------  
                                $chatMsg = array_merge($chatMsg1, $chatMsg2);
                                echo(json_encode(array("status"=>"Success","msg"=> $chatMsg)));   
                             }else{
                              echo(json_encode(array("status"=>"Failed","action"=>"nothing")));       
                             } 
                             }                                                                                                                                                                         
	}
	 ...
	 * 
         * 
	 */
	public function actionChatmsgmanager()
	{
			     if(Yii::app()->user->isGuest || !isset(Yii::app()->session['userid'])){
                                 echo(json_encode(array("status"=>"Failed","action"=>"refresh")));      
                             }else{
                             if(isset($_POST['timestan']) && isset($_POST['to'])){
                              $safaty= new UserManager();                    
                              ...
                              $crc =hash('crc32', $_POST['timestan']);
                              $model =new ChatConversation();
                              $model->chatConverID=(String)($idrand + hexdec($crc64)  + hexdec($crc6) + hexdec($crc));
                              $model->fromID=Yii::app()->session[Yii::app()->session['userid'].'_user'];
                              $model->toID=$_POST['to'];
                              $model->timestampUTC=$_POST['timestan'];
	 ...
	 * 
         * 
	 */
	public function actionNotifBackClosingManager()
	{
			     if(Yii::app()->user->isGuest || !isset(Yii::app()->session['userid'])){
                                 echo(json_encode(array("status"=>"Failed","action"=>"refresh")));      
                             }else{
                                 $criterian = new EMongoCriteria;                               
                                 $criterian->userID =Yii::app()->session[Yii::app()->session['userid'].'_user'];
                                 $criterian->status ="unread";  
                                 $notificationA=Notifications::model()->findAll($criterian);
                                 if (count($notificationA)>0) {
                                     $date= new DateTime(gmdate('Y-m-d H:i:s'));                                                             
                                     $dateCre=$date->format('Y-m-d H:i:s');    
                                     foreach ($notificationA as $notifA) {
                                      if($notifA->notifType!='kolabinvitation' && $notifA->notifType!='squareinvitation'){
                                        $notifA->status ="read"; 
                                        $notifA->decisionDate =$dateCre; 
                                        $notifA->update();
                                      }   
                                     }
                                      echo(json_encode(array("status"=>"Success")));  
	 ...
	 * Manages all models.
	 */
	public function actionKolabInvitationNotifDecision()
	{
			if(isset($_POST['binderid'])){
                            $binderID=  explode('_', $_POST['binderid']);
                            $criterian = new EMongoCriteria;
                            $criterian->notifID =$binderID[0];  
                            $criterian->userID =Yii::app()->session[Yii::app()->session['userid'].'_user'];
                            $criterian->status ="unread";  
                            $notificationA=Notifications::model()->find($criterian);
                            if($notificationA){
                                 //=============================================                                                  
                                  $date= new DateTime(gmdate('Y-m-d H:i:s'));                                                             
                                  $dateCre=$date->format('Y-m-d H:i:s');                                             
                                 ...
                                 //-------------------------------------------------
                                   $userModelA = User::model()->findByPk(Yii::app()->session[Yii::app()->session['userid'].'_user']);
                                   $userModelB = User::model()->findByPk($notificationA->fromID);
                                 //==========================================  
                                 $notificationA->status ="read"; 
                                 $notificationA->decisionDate =$dateCre;
                                 $notificationA->invitationDecision =$_POST['decision'];
                                 
                                            //------------------------------------------------------------
                            $contactmodelA= Contacts::model()->findByPk(Yii::app()->session[Yii::app()->session['userid'].'_user']);
                           if(!$contactmodelA){
                              $contactmodelA = new Contacts;
                              $contactmodelA->userID=Yii::app()->session[Yii::app()->session['userid'].'_user'];
                           }                          
                           $contactListA= new ContactList();                                                  
                           $contactListA->userID= $notificationA->fromID;
                           $contactListA->creationDate=$dateCre;
                           $contactListA->firstname= $userModelB->firstname;
                           $contactListA->lastname=$userModelB->lastname;
                           $contactListA->pictureURL=$userModelB->photoProfile;
                           $contactmodelA->contactList[]=$contactListA;
                           //-----------------------------------------------------
                          
                           $contactmodelB= Contacts::model()->findByPk($notificationA->fromID);
                           if(!$contactmodelB){
                              $contactmodelB = new Contacts;
                              $contactmodelB->userID=$notificationA->fromID;
                           }
                           $contactListB= new ContactList();
                           $contactListB->firstname= $userModelA->firstname;
                           $contactListB->lastname=$userModelA->lastname;
                           $contactListB->userID= $userModelA->userID;
                           $contactListB->creationDate=$dateCre;
                           $contactListB->pictureURL=$userModelA->photoProfile;
                           $contactmodelB->contactList[]=$contactListB;                           
                        
                           //---------------------------------------------------------
                           if($notificationA->update() && $contactmodelB->save() && $contactmodelA->save()){
                               if($_POST['decision']=='Accept'){
                                 $notification= new Notifications();
                                 $idnotif=rand(1,10000000000000);                                                                                         
                                 $crc64no =hash('crc32', $dateCre);
                                 $crc6no =hash('crc32', $notificationA->fromID);                           
                                 $notification->notifID= (String)($idnotif + hexdec($crc64no) + hexdec($crc6no));
                                 $notification->creationDate=$dateCre;
                                 $notification->notifColor='nada';
                                 $notification->status='unread';
                                 $notification->notifType='kolabinvitationaccepted';
                                 $notification->userID=$notificationA->fromID;
                                 $notification->notifText='Invitation accepted';
                                 $notification->from=$userModelA->firstname.' '.$userModelA->lastname;
                                 $notification->fromID=Yii::app()->session[Yii::app()->session['userid'].'_user'];
                           //*****************************************************
	...
	{                
                 ...
                 for ($i = 1; $i <= $number; $i++) {                     
                    ...
                    $subject = Yii::app()->session['username'].' invite you on Kolab'; 
                    ...
                    $message = "Welcome to kolabNX Preview, click on the following link to complete to accept the invitation!\n\n ".$url; 
                    $meoi=Yii::app()->session['username'];
                    //define the headers we want passed. Note that they are separated with \r\n
                    $headers = "From: $meoi <ftoure@nxecorporation.com>\r\nReply-To: ftoure@nxecorporation.com";
                    if (!$model) {                         
                       // echo(json_encode(array("status"=>"Success")));  
                  ...
                  if($success){
                    ...
                    echo(json_encode(array("status"=>"Failled","emails"=>$failledEmail)));    
                  }  
	 ...
	 * 
	 */
	public function actionUpdateSquare()
	{
	
        if(Yii::app()->user->isGuest  || !isset(Yii::app()->session['userid'])){
             echo(json_encode(array("status"=>"Failed","action"=>"refresh")));    
        }else{	
	if(isset($_POST['squareid'])){
                $returnArray=trim(substr($_POST['squareid'],3)); 
                $notifuser= new UserManager();
                $updateSafe=false;
                $notifAreSent=false;
             
              
                $model=Squares::model()->findByPk($returnArray);  
                
                if($model){
                  if($notifuser->roleInSquare($returnArray, Yii::app()->session[Yii::app()->session['userid'].'_user'])=='SquareMaster'){    
                ...
                if($model->description!=$_POST['description']){
                  ...
                  if(isset($_POST['deadline']) && trim($_POST['deadline'])!=''){
                   $deadl=  explode('/', $_POST['deadline']);
                   if(count($deadl)==3){
                     $deadTemp=$deadl[2].'-'.$deadl[1].'-'.$deadl[0].' '.$_POST['hoursdead'].':'.$_POST['mindead'].' '.$_POST['perioddead']; 
                     if($model->deadline!= $deadTemp){
                        $model->deadline= $deadTemp;  
                        $updateSafe=true;  
                     }                    
                   }                                     
                 }
               if($updateSafe){
               $model->lastChangeDate=$dateCre;   
                 $currentBinderID=trim(substr($_POST['binderid'],3));
                 $c = new EMongoCriteria;
                 $c->binderID=$currentBinderID; 
                 $c->userID=Yii::app()->session[Yii::app()->session['userid'].'_user']; 
                 $binderModel= Binders::model()->find($c);
                  foreach ($binderModel->squaresList as $key=>$value) {
                   if($value->squareID==$returnArray){
                       $binderModel->squaresList[$key]->squareName=$model->name;
                       $binderModel->squaresList[$key]->lastUpdate=$dateCre;
                       break;
                   }
                 }
                if($model->update() && $binderModel->update()){                   
                    //--------------------Noitifications -------------------                                     
                        $notifuser->squareUpdateNotif($returnArray, $dateCre);
                   //-------------------------------------------------------
               ...
               /* if(isset($_POST['userinvitecounter'])){                      
                        $notifuser->inviteInSquare($POST, $returnArray, $dateCre); 
                          $updateSafe=true; 
                 }*/  
               
              }   
               if(isset($_POST['newbinderid'])){               
                 $currentBinderID=trim(substr($_POST['binderid'],3)); 
                 $c = new EMongoCriteria;
                 $c->binderID=$currentBinderID; 
                 $c->userID=Yii::app()->session[Yii::app()->session['userid'].'_user']; 
                 $currentBinderModel= Binders::model()->find($c);
                 $newBinderID=trim(substr($_POST['newbinderid'],3)); 
                 $cn = new EMongoCriteria;
                 $cn->binderID=$newBinderID; 
                 $cn->userID=Yii::app()->session[Yii::app()->session['userid'].'_user']; 
                 $newBinderModel= Binders::model()->find($cn);
                 if($newBinderModel){
                   foreach ($model->squareMembers as $key => $value) {
                      if($value->userID==Yii::app()->session[Yii::app()->session['userid'].'_user']){
                          $model->squareMembers[$key]->binderID=$newBinderID;
                          break;
                      }   
                     } 
                     
                   foreach ($currentBinderModel->squaresList as $key=>$value) {
                      if($value->squareID==$returnArray){
                       $squareImage= $binderModel->squaresList[$key];                      
                       unset($binderModel->squaresList[$key]);
                       break;
                       }
                     }
                      
                   if(isset($squareImage)){
                      
                     $newBinderModel->squaresList[]= $squareImage;
                     if($newBinderModel->update() && $currentBinderModel->update() && $model->update()){
                      $updateSafe=true;   
                     }
                   }  
	...
	public function actionUpdateBinder()
	{
		// renders the view file 'protected/views/site/index.php'
		// using the default layout 'protected/views/layouts/main.php'
                     
             if(isset($_POST['binderID'])){
                  if($_POST['binderID']!="b_klb_frame"){
                 $tempval=explode('_', $_POST['binderID']);
                 $returnArray=substr($tempval[1],3);    
                 $model = Binders::model()->findByPk($returnArray);
                 if($model){
                 $date= new DateTime(gmdate('Y-m-d H:i:s'));                                                          
                 $dateCre=$date->format('Y-m-d H:i:s');                                                           
	...
	public function actionInviteInSquare()
	{
		// renders the view file 'protected/views/site/index.php'
		// using the default layout 'protected/views/layouts/main.php'
                     
             if(isset($_POST['counter'])){
                $index=(int)$_POST['counter'];
                $square = Squares::model()->findByPk($_POST['currentid']);
                $failed=array();
                $Success=array();
                while ($index>0) {
                $temp="email".$index;
                $criteria = new EMongoCriteria;  
                $criteria->email =$_POST[$temp];  
                $user = User::model()->find($criteria);
                $helper= new UserManager();                
                if(!$helper->inSquare($square->squareID, $user->userID)){
                    $notification= new Notifications();
                    $idnotif=rand(1,10000000000000); 
                    $date= new DateTime(gmdate('Y-m-d H:i:s'));                                                          
                    $dateCre=$date->format('Y-m-d H:i:s');                 
                    $crc64no =hash('crc32', $dateCre);                    
                    $crc6no =hash('crc32', $_POST[$temp]);                           
                    $notification->notifID= (String)($idnotif + hexdec($crc64no) + hexdec($crc6no));
                    $notification->creationDate=$dateCre;
                    $notification->notifColor='nada';
                    $notification->status='unread';
                    ...
                    $notification->squareID=$_POST['currentid'];
                    $notification->notifText=$square->name;
                    $userModel = User::model()->findByPk(Yii::app()->session['userid']);
                    $notification->from=$userModel->firstname.' '.$userModel->lastname;
                     ...
                     $failed[]=array("touser"=>$_POST[$temp]);
                     }
                     $index= $index-1;
                }    
               
                }
                if(count($failed)==(int)$_POST['counter']){
                     echo(json_encode(array("status"=>"Failed")));    
                }else{                  
                    echo(json_encode(array("status"=>"Success","contentS"=> $Success,"contentF"=> $failed))); 
                }
	...
	public function actionSearchGeneral()
	{
		...
		// using the default layout 'protected/views/layouts/main.php'
             ...
             if(isset($_POST['token'])){
                 $token=$_POST['token'];
                 $criteria = new EMongoCriteria;
                 $criteria->firstname = new MongoRegex("/^$token/i");
                 $criteria->userID('notEq', Yii::app()->session['userid']) ;
                 $criteria2 = new EMongoCriteria;
                 $criteria2->lastname = new MongoRegex("/^$token/i");
                 $criteria2->userID('notEq', Yii::app()->session['userid']);
                 $modelf = User::model()->findAll($criteria);
                 $modell = User::model()->findAll($criteria2);
                  $tosend=array();
                  if(count($modelf)>0){
                   foreach ($modelf as $value) {
                    $tosend[$value->email]=array("id"=> (String)hexdec(hash('adler32', $value->email)),"email"=>$value->email,"photoProfile"=>$value->photoProfile,"fullname"=>$value->firstname.' '.$value->lastname);   
                   }
                  }
                  if(count($modell)>0){
                   foreach ($modell as $value2) {
                    $tosend[$value2->email]=array("id"=> (String)hexdec(hash('adler32', $value2->email)),"email"=>$value2->email,"photoProfile"=>$value2->photoProfile,"fullname"=>$value2->firstname.' '.$value2->lastname);   
                   }
                  }
                 if(count($tosend)>0){                    
                   echo(json_encode(array("status"=>"Success","members"=>$tosend)));      
                 }else{
                     echo(json_encode(array("status"=>"Failed")));  
                 }
	...
	public function actionGetNotifications()
	{
		...
		// using the default layout 'protected/views/layouts/main.php'
                    $criteria = new EMongoCriteria;
                    $criteria->userID =Yii::app()->session[Yii::app()->session['userid'].'_user'];  
                    $userNotif = Notifications::model()->findAll($criteria); 
                    $userNotifsUnread=array();
                    $userNotifsread=array();
                    if(count($userNotif)>0){
                        foreach ($userNotif as $key => $value) {
                          $date1= new DateTime(gmdate('Y-m-d H:i:s'));
                          if(isset(Yii::app()->session['usertimezoneoffset'])){
                            $timezone= Yii::app()->session['usertimezoneoffset'];                         
                            $date1->modify("$timezone hour");  
                          }
                          $date2= new DateTime($value->creationDate);
                          if(isset(Yii::app()->session['usertimezoneoffset'])){
                           $timezone= Yii::app()->session['usertimezoneoffset'];                         
                            $date2->modify("$timezone hour");     
                          }
                         $interval =$date1->diff($date2);
                         $fromPicture="";
                         if(isset($value->fromID) && !is_null($value->fromID)){
                          $fromPicModel=User::model()->findByPk($value->fromID); 
                          if($fromPicModel){
                               $fromPicture=$fromPicModel->photoProfile;
                          }
                         }
                         if($value->status=='unread'){
                           $userNotifsUnread[]=array("from"=>$value->from,"fromPicture"=>$fromPicture,"notifColor"=>$value->notifColor,"notifID"=>$value->notifID,"notifText"=>$value->notifText,"notifType"=>$value->notifType,"status"=>$value->status,"creationDate"=>$interval);    
                         } else {                             
                            $userNotifsread[]=array("from"=>$value->from,"fromPicture"=>$fromPicture,"notifColor"=>$value->notifColor,"notifID"=>$value->notifID,"notifText"=>$value->notifText,"notifType"=>$value->notifType,"status"=>$value->status,"creationDate"=>$interval);        
                         }
                       
                        }   
                    }
                    echo(json_encode(array("unreadnotif"=>$userNotifsUnread,"readednnotif"=>$userNotifsread))); 
	...
	public function actionSetUserSession()
	{
		...
		// using the default layout 'protected/views/layouts/main.php'
                     if(Yii::app()->user->isGuest || !isset(Yii::app()->session['userid'])){
                      ...
                      $dateCre=$date->format('Y-m-d H:i:s');                                             
                      $crc64 =hash('crc32', $dateCre);              
                      $userModel=  UsersOnline::model()->findByPk(Yii::app()->session[Yii::app()->session['userid'].'_user']);
                      if ($userModel) {
                       $userModel->delete();    
                      }
                      $userModel= new UsersOnline;
                      $userSessionModel= new UsersSessions;
                      $userModel->userID=Yii::app()->session[Yii::app()->session['userid'].'_user'];
                      $userModel->sessionID=(String)($id + hexdec($crc64)  + Yii::app()->session[Yii::app()->session['userid'].'_user'] + Yii::app()->session['userid']);
                      $userModel->sessionDate= $dateCre;
                      $userSessionModel->sessionID= (String)($id + hexdec($crc64)  + Yii::app()->session[Yii::app()->session['userid'].'_user'] + Yii::app()->session['userid']);
                      $userSessionModel->userID=Yii::app()->session[Yii::app()->session['userid'].'_user'];
                      $userSessionModel->sessionStartDate=$dateCre;
                      
                      if($userSessionModel->save() && $userModel->save()){
                         // Yii::app()->session['usersessionid']= $userSessionModel->sessionID;
                          echo(json_encode(array("status"=>"Success")));    
                      }                                                                    
                     }
	...
	public function actionGetMyIdentity()
	{
		...
		// using the default layout 'protected/views/layouts/main.php'
                    ...
                    $model=  Contacts::model()->findByPk(Yii::app()->session[Yii::app()->session['userid'].'_user']);
                    $contacts=array();
                    if($model){
                     $contacts= $model->contactList;  
                    }
                     $userModel=  UsersOnline::model()->findByPk(Yii::app()->session[Yii::app()->session['userid'].'_user']);
                     $sessionID='';
                     if($userModel){
                      Yii::app()->session['usersessionid']= $userModel->sessionID;
                       $sessionID=$userModel->sessionID;;
                     }
                     
                    echo(json_encode(array("status"=>"Success","id"=>Yii::app()->session['useremail'],"tokn"=> Yii::app()->session['username'],"tok"=>Yii::app()->session[Yii::app()->session['userid'].'_user'],"contacts"=>$contacts,"usersessionid"=> $sessionID)));     
                   }
	}
	...
	public function actionGetbasicinfos()
	{
		// renders the view file 'protected/views/site/index.php'
		// using the default layout 'protected/views/layouts/main.php'
                  if(Yii::app()->user->isGuest || !isset(Yii::app()->session['userid'])){
                  ...
                  echo(json_encode(array("status"=>"Success","id"=>Yii::app()->session['useremail'],"tok"=>Yii::app()->session['userid'])));         
                 }
	...
	public function actionDeleteBinder()
	{
		// renders the view file 'protected/views/site/index.php'
		// using the default layout 'protected/views/layouts/main.php'                           
               if(isset($_POST['binderID'])){
                 ...
                 $model = Binders::model()->findByPk($returnArray);
                 if($model){                 
                  $dontLockBinderDeletion=true;
                  if(isset($model->squaresList) && !is_null($model->squaresList) && count($model->squaresList)>0){
                      foreach ($model->squaresList as $key => $value) {
                          $quare= Squares::model()->findByPk($value->squareID);
                          if ($quare) {
                            if(count($quare->squareMembers)==1){
                                     $quare->squareMembers[0]->userRole='none';
                                     $posTemp=$quare->squareMembers[0]->userColor;
                                     $quare->squareMembers[0]->userColor=null;
                                     $posListTemp=$quare->availabledcolors;
                                     $date= new DateTime(gmdate('Y-m-d H:i:s'));                                                          
                                     $dateCre=$date->format('Y-m-d H:i:s');
                                     $quare->lastChangeDate=$dateCre;
                                     $quare->availabledcolors= $posListTemp.','.$posTemp;
                                     $quare->status='inactive';
                                     $quare->squareMasterNumber=0;
                                     if(!$quare->update()){
                                     $dontLockBinderDeletion=false;  
                                     }
                            }else if(count($quare->squareMembers)>1){
                                foreach ($quare->squareMembers as $keysq => $valuesq) {
                                    if($valuesq->userID==Yii::app()->session[Yii::app()->session['userid'].'_user']){
                                    
                                    if($valuesq->userRole=='SquareMaster' && ($quare->squareMasterNumber==1 || !isset($quare->squareMasterNumber) || is_null($quare->squareMasterNumber))){
                                      $chooseChief= new UserManager();
                                      $chooseChief->setAnotherSquareMaster($value->squareID);
                                     }else if($valuesq->userRole=='SquareMaster' && $quare->squareMasterNumber>1){
                                      $tempnumber=$quare->squareMasterNumber;
                                      $quare->squareMasterNumber= $tempnumber -1;
                                     }
                                        
                                     $quare->squareMembers[$keysq]->userRole='none';
                                     $posTemp=$quare->squareMembers[$keysq]->userColor;
                                     $quare->squareMembers[$keysq]->userColor=null;
                                     $posListTemp=$quare->availabledcolors;
                                     $quare->availabledcolors= $posListTemp.','.$posTemp;
                                     $date= new DateTime(gmdate('Y-m-d H:i:s'));                                                          
                                     $dateCre=$date->format('Y-m-d H:i:s');
                   ...
                   if($dontLockBinderDeletion) {
                      if($model->delete()){
                       echo(json_encode(array("status"=>"Success","id"=>$tempval[1])));      
                      }else{
                       echo(json_encode(array("status"=>"Failed")));    
                      }    
                   }else{
	 ...
	 * 
	 */
	public function actionDeleteSquare()
        {
          if(isset($_POST['squareID'])){
                        $quare= Squares::model()->findByPk($_POST['squareID']);
                           $lockSquareDeletion=true;
                            if ($quare) {
                            if(count($quare->squareMembers)==1){
                                     $quare->squareMembers[0]->userRole='none';
                                     $posTemp=$quare->squareMembers[0]->userColor;
                                ...
                                foreach ($quare->squareMembers as $keysq => $valuesq) {
                                    ...
                                    if($valuesq->userRole=='SquareMaster' && ($quare->squareMasterNumber==1 || !isset($quare->squareMasterNumber) || is_null($quare->squareMasterNumber))){
                                      $chooseChief= new UserManager();
                                      $chooseChief->setAnotherSquareMaster($_POST['squareID']);                                     
                                      $quare->squareMembers[$keysq]->userRole='none';
                                      $posTemp=$quare->squareMembers[$keysq]->userColor;
                                      $quare->squareMembers[$keysq]->userColor=null;
                                      $posListTemp=$quare->availabledcolors;
                           ...
                           $modelBinder = Binders::model()->findByPk($_POST['binderID']); 
                           if($modelBinder){
                            if(isset($modelBinder->squaresList) && !is_null($modelBinder->squaresList) && count($modelBinder->squaresList)>0){
                                  foreach ($modelBinder->squaresList as $keyb => $valueb) {                                       
                                      if ($valueb->squareID==$_POST['squareID']) {
                                          unset($modelBinder->squaresList[$keyb]);
                                          break;
                                      }
                                    }}   
                           }
                           
                           if(!$lockSquareDeletion){
                             if($quare->update() && $modelBinder->update()){
                              //------------------------- Notification ------------------------------------------
	 ...
	 * 
	 */
	public function actionGetSquareToken()
	{
		// renders the view file 'protected/views/site/index.php'
		// using the default layout 'protected/views/layouts/main.php'
                     
             if(isset($_POST['idsqr'])){
                
                echo(json_encode(array("status"=>"Success","token"=>  hash('gost', $_POST['idsqr']))));   
             }
	 ...
	 * 
	 */
	public function actionSquareView($id)
	{
		// renders the view file 'protected/views/site/index.php'
		// using the default layout 'protected/views/layouts/main.php'
                $model= new stdClass(); 
                $model->userID=Yii::app()->session['userid'];
              $this->render('squareView',array('model'=>$model,'target'=>'_blank'));
	 ...
	 */
	public function actionLocker($action)
	{
		// renders the view file 'protected/views/site/index.php'
		// using the default layout 'protected/views/layouts/main.php'
            if(Yii::app()->user->isGuest || !isset(Yii::app()->session['userid'])){
	 ...
	 */
        ...
        public function actionGetBinderList()
	{
		// renders the view file 'protected/views/site/index.php'
		// using the default layout 'protected/views/layouts/main.php'
            if(Yii::app()->user->isGuest || !isset(Yii::app()->session['userid'])){
              ...
              $binderList=array();
              $tempBinders=$currentUser->getBindersList();
              if(count($tempBinders)>0){
                  foreach ($tempBinders as $value) {
                   $binder= new stdClass(); 
                   $binder->binderID=substr(strtolower($value->name),0,3).$value->binderID;
                   $binder->name=$value->name;
                   $binder->color=$value->color;
                   $binder->style=$value->style;
                   $binderList[]=$binder;                   
                  }
              }
              $this->renderPartial('//user/binderList', array('model'=>$binderList));
              //echo(json_encode(array("status"=>"Success","binders"=> $binderList)));  
            }
	 ...
	 * 
	 */
	public function actionBinderListChange()
	{
	if(Yii::app()->user->isGuest || !isset(Yii::app()->session['userid'])){
          echo(json_encode(array("status"=>"Failed","action"=>"refresh")));    
        } else {	
	if(isset($_POST['binderlist'])){               
               $criteria = new EMongoCriteria;
               $criteria->email =Yii::app()->session['useremail'];  
               $model = User::model()->find($criteria);
               if($model){               
               $list=explode(',', $_POST['binderlist']);
               if (count($list)>1) {                                                                        
                $model->binderList=  $this->listHelper($list); 
                
                if(isset($_POST['bindersortPreference']) && $_POST['bindersortPreference']=='usercustumizedsort'){
                  $model->bindersortPreference='usercustumizedsort';  
                }
                ...
                echo(json_encode(array("status"=>"Failed","action"=>"refresh2")));     
                  }
        ...
        private function listHelper($param) {
             if($param[0]!="b_klb_frame"){
               $tempval0=explode('_', $param[0]);
               $returnArray=substr($tempval0[1],3);    
             }else{
               $c = new EMongoCriteria;
                    $c->name='Personal';
                    $c->userID=Yii::app()->session[Yii::app()->session['userid'].'_user'];
                    $binders2 = Binders::model()->find($c);
                    if($binders2){
                      $returnArray=$binders2->binderID;   
                    }   
             }
          
            if(count($param)>1){
               for($v=1;$v<count($param);$v++){
                 if($param[$v]!="b_klb_frame"){
                 $tempval=explode('_', $param[$v]);
                 $returnArray=$returnArray.','.substr($tempval[1],3);    
                 }else{
                    $c = new EMongoCriteria;
                    $c->name='Personal';
                    $c->userID=Yii::app()->session[Yii::app()->session['userid'].'_user'];
                    $binders = Binders::model()->find($c);
                    if($binders){
                      $returnArray=$returnArray.','. $binders->binderID;   
                    }
                 }
                
                 }  
            }
            return explode(',', $returnArray);
        }
        /**
	 * 
	 * 
	 * 
	 */
	public function actionGetBinderInfo()
	{
	if(Yii::app()->user->isGuest || !isset(Yii::app()->session['userid'])){
          echo(json_encode(array("status"=>"Failed","action"=>"refresh")));    
        } else {	
	if(isset($_POST['binderID'])){                       
               if($_POST['binderID']!="b_klb_frame"){
                 $tempval=explode('_', $_POST['binderID']);
                 $returnArray=substr($tempval[1],3);    
                 $binders = Binders::model()->findByPk($returnArray);
                 }else{
                    $c = new EMongoCriteria;
                    $c->name='Personal';
                    $binders = Binders::model()->find($c);                   
                 ...
                 if($binders){
                  echo(json_encode(array("status"=>"Success","name"=>$binders->name,"style"=>$binders->style,"color"=>$binders->color,"description"=>$binders->description)));      
                 }else{
                  echo(json_encode(array("status"=>"Failed","action"=>"nothing")));    
                 }
	 ...
	 * 
	 */
	public function actionCreateSquare()
	{
        if(Yii::app()->user->isGuest || !isset(Yii::app()->session['userid'])){	
            echo(json_encode(array("status"=>"Failed2","action"=>"refresh")));   
        }else{
	if(isset($_POST['binderid'])){
              
                
                if($_POST['binderid']=="klb"){
                $c = new EMongoCriteria;
                $c->name='Personal';
                $c->userID=Yii::app()->session[Yii::app()->session['userid'].'_user'];
                $binders = Binders::model()->find($c);
               ...
               $safeToken= $safaty->generate_token(25);
               $crc =hash('crc32', $safeToken);
               $squareID=(String)($id + hexdec($crc64) + hexdec($crc) + $binderid);
               $model->squareID= $squareID;
             //  $model->ownerID=Yii::app()->session['userid'];
               //$model->binderID=Yii::app()->session['currentBinderID'];
               $model->name=$_POST['name'];
               $model->description=$_POST['description'];
               ...
               if(isset($_POST['deadline']) && trim($_POST['deadline'])!=''){
                   $deadl=  explode('/', $_POST['deadline']);
                   if(count($deadl)==3){                       
                       $hour=(int)$_POST['hoursdead'];
                       if($_POST['perioddead']=='PM'){
                        $hour=$hour+12   ;
                       }
                       $deadlineString=$deadl[2].'-'.$deadl[1].'-'.$deadl[0].' '.$hour.':'.$_POST['mindead'].':00';   
                       $deadlineDate= new DateTime($deadlineString);
                       if (isset(Yii::app()->session['usertimezoneoffset'])) {
                           $modify=Yii::app()->session['usertimezoneoffset'];
                           $deadlineDate->modify("-$modify hour");  
                       }
                      $deadlineFormat=$deadlineDate->format('Y-m-d H:i:s');      
                      $deadlineTimestamp=$deadlineDate->getTimestamp();        
                      $model->deadline=$deadlineFormat;  
                      $model->deadlineTimestamp= $deadlineTimestamp;
                   }
               ...
               $model->creationDateTimestamp=$date->getTimestamp();
               $model->lastChangeDate=$dateCre;
               $model->squareMasterNumber=1;
               //----------------------------------------------------------
               $bindersSquareList= new SquaresList(); 
               $bindersSquareList->squareID=$model->squareID;
               $bindersSquareList->squareName=$model->name;
               $bindersSquareList->creationDate=$dateCre;
               $bindersSquareList->lastUpdate=$dateCre;
               $bindersSquareList->status='active';
               if(isset($_POST['deadline']) && trim($_POST['deadline'])!='' && isset($deadlineTimestamp)){
               $bindersSquareList->deadlineTimestamp= $deadlineTimestamp;
               }
               $binderModel= Binders::model()->findByPk($binderid);
               $binderModel->squaresList[]=$bindersSquareList;
               $binderModel->lastUpdateDate=$dateCre;
               //-------------------------------------------------------------------
               $members= new SquareMembers();
               $members->binderID=$binderid;             
               $members->userColor="1";        
               $model->availabledcolors="2,3,4,5,6,7,8";
               $members->userRole="SquareMaster";
               $members->userID=Yii::app()->session[Yii::app()->session['userid'].'_user'];
               $fromPicModel=User::model()->findByPk(Yii::app()->session[Yii::app()->session['userid'].'_user']); 
               $members->firstname= $fromPicModel->firstname;
               $members->lastname= $fromPicModel->lastname;
               $members->dateAdhesion=$dateCre;
               $members->photoProfile= $fromPicModel->photoProfile;
               $model->squareMembers[]=$members;
               
              // echo(json_encode(array("status"=>"Success","squareID"=>substr(strtolower($_POST['name']),0,3).$model->squareID.'_'.$binderid)));    
               //-------------------------------------------------------------------
              if($model->save() && $binderModel->update()){
                   //--------------------Noitifications -------------------
                  
                   if(isset($_POST['userinvitecounter'])){                      
                         $safaty->inviteInSquare($_POST, $squareID, $dateCre);                        
                   }
                   //-------------------------------------------------------
                  echo(json_encode(array("status"=>"Success","squareID"=>substr(strtolower($_POST['name']),0,3).$model->squareID.'_'.$binderid))); 
              
                  }else{
                     echo(json_encode(array("status"=>"Failed1","action"=>"nothing")));     
                  }
               
        }else{
                     echo(json_encode(array("status"=>"Failed3","action"=>"nothing")));     
              }                 
                  }  
	 ...
	 * 
	 */
	public function actionCreateBinder()
	{
        if(Yii::app()->user->isGuest || !isset(Yii::app()->session['userid'])){
        echo(json_encode(array("status"=>"Failed","action"=>"refresh")));       
        }else{	
	if(isset($_POST['color'])){
               ...
               $model->lastUpdateDate=$dateCre;
               if($model->save()){
                   echo(json_encode(array("status"=>"Success","binderID"=>$model->binderID)));   
                  }else{
                     echo(json_encode(array("status"=>"Failed","action"=>"nothing")));     
                  }
	...
	public function actionConnectYahoo()
	{
                  ...
                  define('CONSUMER_KEY', "dj0yJmk9NlVrcEZaelRZNHVUJmQ9WVdrOVV6SlBOMWd3TjJVbWNHbzlNQS0tJnM9Y29uc3VtZXJzZWNyZXQmeD1kMw--");  

                  // Your Consumer Secret goes here.  
                  define('CONSUMER_SECRET', "ebf625937975511e6596c30d42d39fda9740013b");  

                  // Your application ID goes here.  
                  define('APPID', "S2O7X07e");  
                  ...
                  foreach($conatct->contacts->contact as $id){
                      ...
                      $email='';
                       ...
                       if($email!=''){
                          $myContact[]=array("fullname"=>$name,"email"=>$email);   
                       }
	...
	public function actionConnectGmail()
	{
            ...
            $callback=Yii::app()->getBaseUrl(true).'/index.php/user/contactsGmail';
	...
	public function actionContactsGmail()
	{
		...
		$userModelB = FriendsToInvite::model()->findByPk(Yii::app()->session['userid']);
                if(!$userModelB){
                 $userModelB = new FriendsToInvite();
                 $userModelB->userID=Yii::app()->session['userid'];
                }
                 $userModelB->friendsList=$friendsList2;
                if($userModelB->save()){
                    $this->redirect(array('contactFriends'));
                } 
	 ...
	 * 
	 */
	public function actionContactFriends()
	{
		
		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);
                $userModelB = FriendsToInvite::model()->findByPk(Yii::app()->session['userid']);                
                $friendsList2 = $userModelB->friendsList;                 
		$this->render('contactsGmail',array(
			'contacts'=>$friendsList2
		));
	}
       /**
	 * 
	 ...
	 * 
	 */
	public function actionLockerupdate($id)
	{
		...
		// $this->performAjaxValidation($model);
		 ...
		 $currentUser= new UserManager();
                 ...
                 $this->renderPartial('//user/locker',array('model'=>$currentUser->getBindersList()), false, false);
            }
	}
	...
	public function actionSocial($id)
	{
		...
		// $this->performAjaxValidation($model);
		 ...
		 $this->renderPartial('social',array('id'=>$id), false, false);
            }
	}
	...
	public function actionHelp($id)
	{
		...
		// $this->performAjaxValidation($model);
		 ...
		 $this->renderPartial('help',array('id'=>$id), false, false);
            }
	}
	...
	public function actionFeedback($id)
	{
		...
		// $this->performAjaxValidation($model);
		 ...
		 $this->renderPartial('feedback',array('id'=>$id), false, false);
            }
	}
	 ...
	 * 
	 */
	public function actionUserbsInformation($id)
	{
		...
		// $this->performAjaxValidation($model);
		 ...
		 $this->renderPartial('userbsInformation',array('id'=>$id), false, false);
            }
	}
	 ...
	 * 
	 */
	public function actionUsernameChecker()
	{
	...
	public function actionCreate()
	{
		...
		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);
            //preg_match_all ( string $pattern , string $subject ) --- $pattern = '/[!@#$%^&*()]/' 
		...
		if(isset($_POST['password']))
		{
			$criteria = new EMongoCriteria;
                        ...
                        if(!$userModel){
                           ...
                           $dateCre=$date->format('Y-m-d H:i:s');                                             
                           $crc64 =hash('crc32', $dateCre);
                           $crc6 =hash('crc32', $_POST['email']);
                           $userModel->userID=(String)($id + hexdec($crc64) + hexdec($crc6));  
						   $userModel->email=$_POST['email'];
                        }
                           
                           $userModel->adhesionDate=$dateCre;
                           $userModel->firstname=$_POST['firstname'];
                           $userModel->lastname=$_POST['lastname'];
                           $userModel->username=$_POST['username'];                          
                           $userModel->userTimeZoneOffset=$_POST['timezone'];
                           //$userModel->city=$_POST['city'];
                           $userModel->profilComplet=2;
                           
                           if(isset(Yii::app()->session['invitedBy'])){
                             $userModel->invitedBy= Yii::app()->session['invitedBy'];  
                           }
                           //--------------------------------------------------
                           $authModel=Userauthentification::model()->findByPk($userModel->userID);
						   if(!$authModel){
						   $authModel=new Userauthentification;
                           $authModel->userID=$userModel->userID;   
						    $authModel->username=$userModel->email;
						   }						                             
                           $authModel->lastModification=$dateCre;
                           $authModel->password=hash('whirlpool', $_POST['password']);                                                   
                           //-----------------------------------------------------
                           
                           if($userModel->save() && $authModel->save()){
                             
                             $modelSubscriber=Subscribe::model()->findByPk($_POST['email']);
                             $sucees=true;
                              if($modelSubscriber){                                                               
                              if($modelSubscriber->delete()){                                                          
                               ...
                               $criteria2 = new EMongoCriteria;
                               $criteria2->friendEmail =$_POST['email'];                                                           
                               $modelPendingFriends=PendingFriends::model()->findAll($criteria2);
                             //****************************************************
                              if(count($modelPendingFriends)>0 ){
                                foreach ($modelPendingFriends as $value) {
                                 $userModel2 = User::model()->findByPk($value->userID);
                                 $notification= new Notifications();
                                 $idnotif=rand(1,10000000000000);                                                                                         
                                 $crc64no =hash('crc32', $dateCre);
                                 $crc6no =hash('crc32', $userModel2->email);                           
                                 $notification->notifID= (String)($idnotif + hexdec($crc64no) + hexdec($crc6no));
                                 $notification->creationDate=$dateCre;
                                 $notification->notifColor='nada';
                                 $notification->status='unread';
                                 $notification->notifType='kolabinvitation';
                                 $notification->userID=$userModel->userID;
                                 $notification->notifText='invitation request';
                                 $notification->from=$userModel2->firstname.' '.$userModel2->lastname;
                                 $notification->fromID=$value->userID;
                                 $notification->save();
                             }
                                 
                           //*****************************************************
                              if(PendingFriends::model()->deleteAll($criteria2)){
                                  unset(Yii::app()->session['invitedBy']);  
								  Yii::app()->session['useremail']=$userModel->email;
                              }else{
                                   $sucees=false;     
                              }                                                                                          
	 ...
	 */
	public function actionGetSquareInfo()
	{
		if(Yii::app()->user->isGuest || !isset(Yii::app()->session['userid'])){
                 echo(json_encode(array("status"=>"Failed","action"=>"refresh")));    
                 } elseif($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id']))
		  {		
                $returnArray=substr($_POST['id'],3);   
                
               $model=Squares::model()->findByPk($returnArray);                
                if($model){
                   $summer = new DateTime('now', new DateTimeZone($_POST['tz'])); 
                   $timezone=$summer->getOffset()/3600;                
                   $squareinfo= new stdClass();
                    foreach ($model->squareMembers as $key => $value) {
                      if($value->userID==Yii::app()->session[Yii::app()->session['userid'].'_user']){
                        $binder = Binders::model()->findByPk($value->binderID);
                        $squareinfo->name=$model->name;
                        $squareinfo->description=$model->description;
                        if(isset($model->deadline) && !is_null($model->deadline)){                      
                              $squareinfo->deadlinedate=$model->deadlineTimestamp;
                            }
                        $createDate= new DateTime($model->creationDate);                                          
                        $createDate->modify("$timezone hour");                                              
                        $squareinfo->creationDate=$createDate->format('d/m/y');
                        $squareinfo->userRole=$value->userRole;
                        $squareinfo->binderColor=$binder->color;
                        $squareinfo->binderStyle=$binder->style;
                        $squareinfo->binderName=$binder->name;  
                        $squareinfo->binderID=substr(strtolower($binder->name),0,3).$value->binderID; 
                        break;
                       
                      }  
                    }
                    
                    $squareinfo->currentID=Yii::app()->session[Yii::app()->session['userid'].'_user'];  
                    $squareinfo->members=$model->squareMembers;
                    
                    
                 echo(json_encode(array("status"=>"Success","squareinfo"=>$squareinfo)));   
                }else{
                  echo(json_encode(array("status"=>"Failed")));    
                }
                }else{
	 ...
	 * 
	 */
	public function actionCreateAfterInvitation()
	{	
		if(isset($_POST['password']))
		{
			$criteria = new EMongoCriteria;
                        ...
                        if($userModel){
                          echo(json_encode(array("status"=>"Failed")));    
                           ...
                           $authModel->userID=$userModel->userID;
                           $authModel->username=$userModel->email;
                           $authModel->lastModification=$userModel->adhesionDate;
                           $authModel->password=hash('whirlpool', $_POST['password']);
                           //------------------------------------------------------------
	 ...
	 * 
	 */
	public function actionGetInviteInSquareInfos()
	{
		$userModel = User::model()->findByPk($_POST['fromUser']);
                $squareModel = Squares::model()->findByPk($_POST['squareID']);
                
                if($userModel && $squareModel){
                  $squareInvitationInfo=array(); 
                  $squareInvitationInfo['from']=$userModel->firstname.' '.$userModel->lastname;
                  $squareInvitationInfo['frompicture']=$userModel->photoProfile;
                  $squareInvitationInfo['squarename']=$squareModel->name;
                  echo(json_encode(array("status"=>"Success","squareInvitationInfo"=>$squareInvitationInfo)));  
	 ...
	 * 
	 */
	public function actionTimezoneoffsettest()
	{
                ...
                if(isset($_POST['tz'])){                  
                  ...
                  Yii::app()->session['usertimezoneoffset']=$summer->getOffset()/3600;
                  Yii::app()->session['usertimezone']=$_POST['tz'];
                  echo(json_encode(array("status"=>"Success","offset"=>$summer->getOffset()/3600,"timestamp"=>$summer->getTimestamp()))); 
                }else{
                 echo(json_encode(array("status"=>"Failed1")));    
                }
	...
	public function actionCreateContact()
	{
		...
		// using the default layout 'protected/views/layouts/main.php'
		    ...
		    {
                    $modelNotif= Notifications::model()->findByPk($_POST['idnotif']);
                    if($modelNotif){
                        $modelNotif->status='read';
                        if($_POST['decision']=='1'){                         
                         $date= new DateTime(gmdate('Y-m-d H:i:s'));                                                          
                         $dateCre=$date->format('Y-m-d H:i:s');
                       //*************************************************************************************   
                         $contactmodelA=  Contacts::model()->findByPk(Yii::app()->session['userid']);
                         $userModelA = User::model()->findByPk(Yii::app()->session['userid']);
                         $userModelB = User::model()->findByPk($modelNotif->fromID);
                        if(!$contactmodelA){
                              $contactmodelA = new Contacts;
                              $contactmodelA->userID=Yii::app()->session['userid'];
                          }
                           $contactListA= new ContactList();                                                     
                           $contactListA->userID= $userModelB->userID;
                           $contactListA->creationDate=$dateCre;
                           $contactListA->firstname= $userModelB->firstname;
                           $contactListA->lastname=$userModelB->lastname;
                          $contactListA->pictureURL=$userModelB->photoProfile;
                           $contactmodelA->contactList[]=$contactListA; 
                        //*************************************************************************************
                         $contactmodelB=  Contacts::model()->findByPk($modelNotif->fromID);                        
                          if(!$contactmodelB){
                              $contactmodelB = new Contacts;
                              $contactmodelB->userID=$modelNotif->fromID;
                           }
                           $contactListB= new ContactList();
                           $contactListB->firstname= $userModelA->firstname;
                           $contactListB->lastname=$userModelA->lastname;
                           ...
                           $contactListB->pictureURL=$userModelA->photoProfile;
                           $contactmodelB->contactList[]=$contactListB;      
                   if($contactmodelB->save() && $contactmodelA->save()){
                        // $modelNotif->update();
                         $notification= new Notifications();
                         $idnotif=rand(1,10000000000000);                                                                                         
                         $crc64no =hash('crc32', $dateCre);
                         $crc6no =hash('crc32', $userModelA->userID);                           
                         $notification->notifID= (String)($idnotif + hexdec($crc64no) + hexdec($crc6no));
                         $notification->creationDate=$dateCre;
                         $notification->notifColor='nada';
                         $notification->status='unread';
                         $notification->notifType='kolabinvitationaccepted';
                         $notification->userID=$userModelB->userID;
                         $notification->notifText='Invitation accepted';
                         $notification->from=$userModelA->firstname.' '.$userModelA->lastname;
                         $notification->fromID=$userModelA->userID;
                         if($notification->save()){
                          echo(json_encode(array("status"=>"Success","notifTo"=>$userModelB->userID)));   
                         }else{
                           echo(json_encode(array("status"=>"Failed1")));    
                         }
